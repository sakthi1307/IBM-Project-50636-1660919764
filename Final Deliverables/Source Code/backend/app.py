...
def put_record(current_user):
    ...
    dt = datetime.utcnow()
    record_this_month = Record.query.filter_by(user=current_user.public_id).filter(db.extract(
    ...
    if current_month_spending >= current_user.monthly_limit:
        ...
        <img src="https://image.shutterstock.com/image-vector/white-coupon-banner-word-over-260nw-2213547155.jpg" alt="over budget!"/>
...
def dashboard(current_user):
    ...
    record_this_month = Record.query.filter_by(user=current_user.public_id).filter(db.extract(
        'year', Record.date_created) == dt.year, db.extract('month', Record.date_created) == dt.month)
    record_last_seven_days = Record.query.filter_by(user=current_user.public_id).filter(
    ...
    current_month_spending = sum(
        [-1*i.amount if i.gain else i.amount for i in record_this_month.all()])
    if dt.month > 1:
        last_month_spending = Record.query.filter_by(user=current_user.public_id).filter(db.extract(
            'month', Record.date_created) == dt.month-1, db.extract('year', Record.date_created) == dt.year).all()
    else:
        last_month_spending = Record.query.filter_by(user=current_user.public_id).filter(db.extract(
    ...
    for x, y in by_category:
        category_list[x] = y
    response_obj = make_response(jsonify({'last_week_spending': last_week_spending, 'expense_by_category': category_list, 'income': income, 'balance': balance,
                                 'monthly_limit': current_user.monthly_limit, 'current_month_spending': current_month_spending, 'last_month_spending': last_month_spending}), 201)
